...
WATCHLIST = [
    ...
    # ── 日本株ADR（米国市場での日本企業そのもの）──
    # 時間外の動きが翌朝の東京市場の同じ銘柄にほぼ直結する。最優先で監視する。
    ("SONY",  "ソニーG(ADR)",     "日本株ADR"),
    ("TM",    "トヨタ(ADR)",      "日本株ADR"),
    ("MUFG",  "三菱UFJ(ADR)",     "日本株ADR"),
    ("SMFG",  "三井住友FG(ADR)",  "日本株ADR"),
    ("HMC",   "ホンダ(ADR)",      "日本株ADR"),
    ("MFG",   "みずほFG(ADR)",    "日本株ADR"),

    # ── 日本の対応銘柄が明確な米国株 ──
    ("GE",    "GEエアロスペース", "資本財"),     # IHI・三菱重工
    ("DE",    "ディア",           "資本財"),     # クボタ
    ("FCX",   "フリーポート",     "資源"),       # 住友金属鉱山・商社
    ("C",     "シティグループ",   "金融"),       # メガバンク
    ("PFE",   "ファイザー",       "医薬"),
    ("NKE",   "ナイキ",           "消費"),       # アシックス・ゴールドウイン
    ("MCD",   "マクドナルド",     "消費"),       # 日本マクドナルドHD
    ("FDX",   "フェデックス",     "物流"),       # 景気の先行指標としても重要
    ("UPS",   "UPS",              "物流"),
    ("UNP",   "ユニオンパシフィック", "運輸"),
]
