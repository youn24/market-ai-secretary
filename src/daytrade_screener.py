...
# ⚠️ 2026-09-09〜09-11 の間、売買代金8,000億円超に「?」（数字が怪しい）を付けていた。
#    キオクシアが9日連続で1.1〜2.0兆円と出ており、「東証全体が1日4〜5兆円なので
#    1銘柄でこれはあり得ない」と判断したため。
#    **これは誤りだった。** 09-11 に株探（別の取得元）で突き合わせたところ、
#    前場途中の時点で既に8,117億円、株価53,690円と一致した。
#    メモリ株が異常に買われている相場で（米マイクロンも2025年の約10倍）、
#    キオクシアが売買の主役になっているだけで、データは本物だった。
#    「普段の常識」で外れ値を決めると、相場が変わったときに本物を疑うことになる。
#    本物の数字に「怪しい」と付けて利用者に不要な疑いを持たせるのは、
#    嘘の数字を出すのと同じくらい害があるので、印は外した。
