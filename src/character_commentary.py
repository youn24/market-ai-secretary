...
【最優先で厳守・正確性】
・下に与えられた【市場データ】【セクター/業種】【詳しい材料】【ニュース】に実際に書かれている
　事実・数値・銘柄名・指標結果のみを使うこと。そこに無い数値・銘柄・決算・イベントを
　推測やそれっぽさで創作しては絶対にならない。
・データが与えられていない事柄には触れず、その話題は省く（無い決算・無い指標を語らない）。
・数値は与えられた値をそのまま尊重し、勝手に変えたり盛ったりしない。
・断定は避け、不確実なことは「〜の公算」「〜の可能性」と留保する。
...
6. 【重要・具体名で】注目銘柄：与えられた材料（動いた銘柄・手法シグナル・材料分析・
   TDnet・スクリーナー・テーマ株・需給・ADR等）の中から、今日とくに注目に値する銘柄を
   3〜5つ、必ず実際の固有名で挙げ、それぞれ「なぜ注目か（動いた理由・シグナル・材料）」を
   一言添える。※データに載っている銘柄のみ。載っていない銘柄を創作しないこと。
   売買を勧める断定はせず「注目」「一考の余地」の水準に留める。
7. 需給とセンチメント：F&G・VIX・リスクスコアから市場心理を読む
8. 時間帯戦略：寄り付き・前場・後場で想定される値動きの型を述べる
...
【詳細】
（ここに 1300〜1600字 の詳細分析。20〜26文。うちセクター/物色の論述に約3分の1、
　注目銘柄の具体的な言及にも十分な分量を割く。上記14項目をできるだけ盛り込む。
　ただし箇条書きにはせず、流れるような一続きの語りにし、最後まで濃度を保つこと）
【ひとこと】
...
今回は特に: {otter_flavor}。
【最優先で厳守】材料に実際に書いてある数字や会社の名前だけを使ってね。
書いていない数字・会社・決算を、想像で作っちゃダメだよ。無いことは無理に言わないでね。
...
　③だから日本の株はどうなりそう？（材料に出ている会社があれば、その名前を実際に出して、
　　　なぜ元気なのか理由もセットでかわいく紹介。※材料に無い会社は作らないでね）
...
【出力形式（この2ブロックを必ずこの順で、見出し記号もそのまま出力）】
【詳細】
（ここに 700〜850字。13〜18文。5つのポイントを順にかわいく、たっぷり丁寧に説明）
【ひとこと】
