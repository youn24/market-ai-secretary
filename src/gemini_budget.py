...
# ⚠️ 2048 は思考モデルには狭すぎる。gemini-2.5-flash は答える前に
#    内部推論で出力トークンを使うため、2048だと思考だけで使い切り、
#    本文が空のまま打ち切られる（ai_brief が2週間これで落ちていた）。
#    無料枠の制約は「1日20回」であってトークン数ではない
#    （250,000トークン/日に対し 20回×4096 でも余裕がある）ので、
#    ここを絞っても得るものが無い。
MAX_OUTPUT = int(os.getenv("GEMINI_MAX_OUTPUT", "4096"))
...
CORE = {
    ...
    # 2026-09-10: 1回 → 2回。朝の通知と公開レポートの主役がこれなので、
    # 1回失敗したら中身が丸ごと消える。実際 08-27〜09-10 の9回すべて
    # 「思考トークンで打ち切られて本文が空」で落ち、2週間気づけなかった。
    # 2回目はスキーマ指定を外した再試行で、**失敗した日しか使わない**。
    "ai_brief":           2,   # 3視点＋3シナリオ＋変動要因を1回で（+失敗時の再試行1回）
...
NICE = {
    ...
    # 2026-08-28: 1材料=1呼び出しを、全材料を1回にまとめる形へ変更した。
    # 実測 6回3,970トークン → 1回1,707トークン。
    # しかも従来は3回で打ち切られ材料を取りこぼしていたのが、全件見られる。
    # 2 なのは、一括が失敗した日に個別版の受け皿が1回動けるようにするため。
    "catalyst_analyzer":    2,  # 材料分析（一括1回＋受け皿1回）
