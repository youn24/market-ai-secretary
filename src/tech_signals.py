...
def _mtf_text(m: dict) -> list:
    ...
    if m["full"]:
        out.append(f"→ ⭐ *全時間軸が{_MTF_JA[m['direction']]}向き*。"
                   f"流れが揃った分かりやすい形です"
                   f"（ただし検証では、揃っているから当たりやすいとは言えませんでした）。")
    elif m["direction"] == "flat":
        out.append("→ ⚠️ *どの時間軸も方向が定まっていません*。"
                   "過去3,855件で唯一はっきり成績が悪かった状態です"
                   "（相場全体の上昇分を除いて20日後 -1.18%）。")
    elif m["conflict"]:
...
def _confluence(hits: list) -> list:
    ...
    for g in by_sym.values():
        ...
        # 反映の仕方は過去3,855件の検証に基づく（src/mtf_validation.py・2026-08-18）。
        #
        # 重要: 生のリターンで比べると「全時間軸一致」が20日後+1.05%で最良に見えたが、
        # これは2022〜2026が上昇相場で、全銘柄の20日平均が+1.51%あったことの反映だった。
        # 銘柄ごとの平均リターン（何もしなくても得られる分）を差し引くと
        # 全時間軸一致の優位は -0.23%（p=0.18）となり、実力とは言えなくなる。
        # そのためランクの格上げは行わない。表示はするが、当たりやすいとは言わない。
        #
        # ドリフトを差し引いても残った唯一の傾向が「中立（どの足も方向感なし）」の
        # 弱さ（5日 -0.56% p=0.014 / 20日 -1.18% p=0.014）。ここだけ格下げする。
        # 逆行はむしろ僅かにプラスで、下げる根拠が無いため触らない。
        def _demote(s, r):
            if r == "最高":  return "★★", "中"
            if r == "高":    return "★★", "中"
            if r == "中":    return "★",  "参考"
        ...
        if mtf:
            ...
            if mtf["full"] and mtf["direction"] == want:
                # 揃っている事実は伝えるが、ランクは上げない（実力が確認できないため）
                g["mtf_full"] = True
            elif mtf["direction"] == "flat":
                # ドリフト調整後も有意に弱い唯一の状態。ここだけ格下げする
                stars, rank = _demote(stars, rank)
                g["mtf_flat"] = True
            elif mtf["direction"] != want:
                # 逆行は調整後わずかにプラス。下げる根拠が無いので印だけ残す
                g["mtf_against"] = True
...
def build_message(hits: list) -> str:
    ...
    for g in groups:
        ...
        if g.get("mtf_against"):
            lines.append("ℹ️ 大きな時間軸とは逆向きですが、検証では成績に差が無かったため"
                         "信頼度は下げていません。")
        if g.get("mtf_flat"):
