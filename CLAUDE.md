...
| `src/sakata.py` | 酒田五法（三山・三川・三空・三兵・三法）の検出。効果は未確認 |
| `src/candlestick.py` | ローソク足7種＋プライスアクション3種の検出と通知（効果は未確認・記述用） |
| `src/pattern_validation.py` | 形の効果検証（27,994件・ドリフト調整＋多重比較補正） |
| `src/gap_scanner.py` | 窓開け検出（前日終値↔始値。埋め具合まで判定・統計しきい値） |
...
| `.github/workflows/health_check.yml` | UTC 00:30 = JST 9:30（平日）＋ワークフロー変更時の自動検査 |
...
## 🕯 ローソク足・プライスアクション（2026-08-19追加）

`src/candlestick.py` で7種のローソク足＋3種のプライスアクションを検出。
**ただし効果は確認できていない。** 通知は「今どんな形か」の記述であって予測ではない。

### 設計の要点
1. **文脈のない形は数えない** … ハンマーは「下落したあと」でないと意味がない。
   文脈を無視すると毎日どこかでシグナルが出て意味を失う
2. **大きさを問う** … 実体がATRの半分未満の包み足は値が動いていないのと同じ
3. **「状態」と「出来事」を分ける** … 高値安値の切り上げは数週間続く*状態*。
   出来事として数えるとローソク足1本で「2つ重なり」になる。
   実測でも状態を含めると重なりが年30回近くに水増しされた（`_STATE_PATTERNS`）

通知は実測頻度に合わせた2階層（6銘柄×2年＝2,624日で計測）:
出来事2つ以上＝0.76%の日（年2回ほど・⭐）／ 出来事1つ＋構造一致＝4.2%の日

### ⚠️ 検証結果（27,994件・30銘柄・5年）— 期待とは違った
ドリフト調整後、**16種のうち補正後も有意なのは2種だけで、どちらも逆の結果**。

| パターン | n | 実力(5日) | p値 | 判定 |
|---|---|---|---|---|
| 高値・安値ともに切り下げ | 9,367 | **-0.15%** | 0.0001 | ✅補正後も有意・**逆効果** |
| 明けの明星 | 168 | **-1.09%** | 0.002 | ✅補正後も有意・**逆効果** |
| 弱気のはらみ足 | 365 | +0.65% | 0.015 | △補正前のみ |
| ダブルボトム | 94 | +0.63% | 0.040 | △補正前のみ |

**重なりの効果も確認できなかった**:
1つだけ -0.06% ／ 2つ重なり **+0.01%**(n=1,827, p=0.96) ／ 3つ以上 +0.53%(n=131, p=0.31)

### 🎌 酒田五法の検証結果（2026-08-19・29,091件）
本間宗久の五法を `src/sakata.py` で実装し、同じ基準で検証した。

| 形 | n | 実力(5日) | p値 |
|---|---|---|---|
| 逆三尊（三川） | 82 | +0.82% | 0.114 |
| 赤三兵 | 248 | +0.28% | 0.369 |
| 三羽烏（黒三兵） | 190 | +0.08% | 0.846 |
| 三尊天井（三山） | 131 | +0.00% | 0.994 |
| 三空踏み上げ | 277 | −0.13% | 0.694 |
| 三空叩き込み | 140 | −0.22% | 0.714 |

**五法のどれも統計的に有意ではなかった。** 逆三尊が最も良い数字（+0.82%）だが
n=82・p=0.114 で、偶然の範囲を出ない。

⚠️ 酒田五法は年に数回しか出ないため、**単独でも通知する**例外を設けてある
（三尊天井などは重なりを待つと手遅れになるため）。ただし通知文では
「効果は確認できていません」と明記する。

⚠️ 実装時のつまずき: 三兵の「前の足の実体内で寄り付く」を厳密に取ると
2年6銘柄で2回しか成立しなかった。三法も完全内包を求めると0回。
五法が排除したいのは「飛び離れた寄り付き」なので、ATRの2〜3割の許容を入れて
現実的な頻度（赤三兵14回・三法2回）にした。

### 🚨 多重比較の罠（この検証で新たに学んだこと）
16種類を検定すれば、**効果がゼロでも平均0.8種は p<0.05 になる**（0.05×16）。
補正前は5種が「有意」に見えたが、Bonferroni補正（p<0.00313）を通ると2種に減り、
その2種はどちらも「教科書と逆」だった。

**今後パターンを複数同時に検定するときは必ず補正すること。**
個別のp値だけを見て「有意なパターンを5つ発見」と言うのは、
宝くじを16枚買って当たった1枚だけ見せるのと同じ。

⚠️ 検定は n≥200 で正規近似に切り替えてある。数万件の系列で6,000回の
並べ替えを回すと処理が終わらなかったため（結論は変わらない）。

## 🪟 窓開けと株価注意報（2026-08-19追加）

### `src/gap_scanner.py` — 窓開け
前日終値と当日始値の差＝夜のうちに前提が変わった証拠。
**開けた事実だけでなく「埋め具合」まで測る**のが肝で、同じ窓でも読み方が正反対になる。
- 窓を維持 → 新しい水準が受け入れられた
- 埋め戻した → 寄り付きの動きが行き過ぎだった

しきい値は銘柄ごとに過去1年の窓の大きさから自動計算（risk_gauges と同じ方式）。
新興株とディフェンシブ株では同じ3%の重みが違うため固定値は使わない。

⚠️ **実行タイミング**: 始値が確定しないと測れないので monitor の **9:15〜11:59** のみ。
9:00ちょうどだと気配値を拾うことがある。朝7:30のレポートでは前営業日の窓が出る。

### `src/kabutan_warning.py` — 株探・株価注意報
`https://kabutan.jp/warning/?mode=XX` から取得。株ドラゴンとの住み分けは:
- **株ドラゴン** = 値動きの順位（値上がり率・ストップ高・出来高急増）
- **株探注意報** = その裏側の需給（信用残・年初来更新・日経寄与度）

重複するストップ高・値上がり率は**取らない**（二重通知は読み飛ばしの原因）。

| mode | 内容 |
|---|---|
| 3_3 / 3_4 | 年初来高値 / 安値の更新 |
| 3_2 | ストップ安 |
| 7_1 / 7_2 | 信用売り残 / 買い残の増加（**信用倍率も同時に取れる**） |
| 8_1 | 日経平均への寄与度 |

**信用倍率の読み方**: 買い残÷売り残。10倍超は買い方に偏り上値が重い、
1倍未満は売り方が多く踏み上げが起きやすい。

⚠️ 表のヘッダは列数と一致しないので**見出しに頼らない**。
「先頭セルが4桁英数（証券コード）の行」だけを拾う方式にしてある。

## ⏰ 点検の時刻設定（2026-08-19の誤検知から）

`health_check.yml` は当初 JST 8:30 だったが誤検知した。
朝のレポートは cron 7:30 → 実際の開始は15分ほど遅れ → 生成に約20分 →
さらに GitHub Pages の再ビルドに数分。8:30 では間に合わない。

**対策は2段構え**（時刻だけずらしても実行遅延は吸収できないため）
1. 実行を JST 9:30 に後ろ倒し
2. `publish_check` に再試行を追加。**古い内容だったときだけ** 90秒×3回まで
   待って再確認する。404や中身の破損は待っても直らないので即座に異常と判定

⚠️ 誤報が続くと点検そのものが読み飛ばされ、本物の異常を見逃す。
点検を足すときは「鳴らない工夫」まで含めて設計すること。

## 🚨 アラートのエスカレーション（2026-08-19追加）

原則は変わらず「同じ銘柄は1セッション1回だけ」。
**例外は1つだけ**: 1回目の通知より **1.5倍以上悪化**したとき、
セッション中に**もう1度だけ**知らせる（`_ESCALATE_FACTOR=1.5` / `_ESCALATE_MAX=1`）。

**きっかけ**: 2026-08-18に日経先物が -2.6% で発報したあと -4.0% まで悪化したが、
「1日1回」の決まりで2回目が出せず、事態の悪化が伝わらなかった。

**実装時に踏んだ罠（いずれもテストで発覚）**
1. **浮動小数点**: `2.6*1.5 = 3.9000000000000004` となり、ちょうど1.5倍の
   -3.9% が「未満」と判定されて発報しなかった。`- 1e-9` の許容を入れた
2. **共通台帳が握り潰す**: `notify_ledger.filter_new` は「同じ話題を二度送らない」
   仕組みなので、意図した再通知まで確実に消していた。
   → エスカレーションは台帳を通さないよう分岐
3. **再通知枠が翌日に持ち越される**: 保存時にセッションを見ずに `escalated` を
   引き継いでいたため、昨日1回使うと今日の悪化を知らせられなくなっていた

**誤発報しない設計**
- 記録する変化率はセッション中の**最大値**（直近値で上書きすると、いったん戻した
  あと同じ水準に達しただけで再通知が出る）
- 逆方向への振れ（-3%→+4.6%）は「悪化」ではないので対象外
- 3回目は出ない（オーナー指示「何度もしないでください」を維持）

## 📖 通知の「分析要約」（2026-08-18追加）

3つのシグナル通知すべてに、詳細の前へ *ひとことで言うと* を置く。
数字や個別シグナルを並べるだけでは、読者が自力で総合する負担が大きく、
結局「で、どうすれば？」で終わってしまうため。

| 通知 | 要約の中身 |
|---|---|
| `risk_gauges` | リスクオフ/オン判定（恐怖指数・金利・円の向きを数えて多数決）＋VIXの期間構造 |
| `fx_signals` | 通貨の強弱（クロス円が揃えば「円が全面安」、ドルストレートが揃えば「ドル高」） |
| `tech_signals` | 方向が揃っているか割れているか＋全時間軸一致/逆行の銘柄名 |

**個別指標にも3点セットを付ける**（risk_gauges）:
`📌 これは何？` → `💭 どういうこと？` → `📊 どれくらい大きい？`
「VIX9D +20%」だけでは初心者に伝わらない。何を測る指標で、上がったことが
何を意味し、それが普段と比べて大きいのかまで書いて初めて判断材料になる。

**VIXの期間構造判定**: 通常は VIX9D < VIX < VIX3M（先の方が不透明）。
逆転して目先が3ヶ月先を上回るのは相場の急変局面で現れる形なので明示する。
数字を並べるだけでは見えない情報。

⚠️ 要約はルールベース（Gemini不使用）。15分ごとの監視でAPIを叩くと
遅延とコストが増えるうえ、判定が実行ごとにぶれるため。

## 🔬 テクニカルシグナルの実証（2026-08-18・src/mtf_validation.py）

朝に入れたMTF判定を同日中に検証した結果、**2回作り直した**。その経緯ごと残す。
同じ失敗を繰り返さないために、結論だけでなく途中の誤りも書いておく。

### 検証方法
23銘柄（指数・為替・個別株・商品）×5年、8種のシグナル3,855件。
その日までの終値だけで判定（週足は日足から合成。週足データを直接使うと
週の途中でその週の確定値を見てしまい未来を覗くことになる）。
有意性は並べ替え検定・符号反転検定（5,000〜8,000回）で確認。

### ⚠️ 踏んだ罠その1: 小サンプル
最初に6銘柄で試すと「完全一致は逆行より+1.87%良い」と出た。
23銘柄に増やすと差はほぼ消えた（-0.03%）。**n=33の偶然だった。**

### ⚠️ 踏んだ罠その2: 相場のドリフト（これが最大）
生のリターンでは以下のように見えていた。

| | 20日後 |
|---|---|
| 買いシグナル全体 | +1.50% |
| 売りシグナル全体 | -1.73%（＝株価が上昇＝失敗） |
| 全時間軸一致 | +1.05%（他より良い・p=0.000） |

**しかし2022〜2026は上昇相場で、シグナルと無関係な全銘柄の20日平均が
+1.51%あった。** エヌビディア+6.05%、ドル円+0.35%と銘柄差も大きい。
銘柄ごとの平均リターンを差し引くと結論が反転する。

| シグナル | 生の20日 | **ドリフト調整後** | 有意性 |
|---|---|---|---|
| macd_gc | +1.91% | +0.35% | p=0.39 ❌ |
| high52w | +1.54% | -0.17% | p=0.29 ❌ |
| golden_cross | +0.67% | -0.86% | p=0.16 ❌ |
| dead_cross | -1.04% | +0.44% | p=0.42 ❌ |
| low52w | -3.35% | -1.60% | p=0.06 ❌ |

**結論: 8種のシグナルすべて、持ち続けるのに対する優位性は確認できなかった。**

### 時間軸判定（ドリフト調整後）
| グループ | 5日 | 10日 | 20日 | 判定 |
|---|---|---|---|---|
| 完全一致 (n=2257) | -0.10% | -0.08% | -0.23% | 差なし |
| 一致 (n=778) | -0.07% | -0.07% | +0.22% | 差なし |
| **中立 (n=265)** | **-0.56% ✅p=.014** | -0.58% | **-1.18% ✅p=.014** | **唯一の有効な発見** |
| 逆行 (n=555) | +0.19% | +0.69% ✅ | +0.13% | 差なし（10日のみ） |

### 最終的な実装（3回目）
- **完全一致でランクを上げない** … 調整後の優位が確認できないため。表示はするが
  「当たりやすい」とは書かない
- **逆行でランクを下げない** … 調整後はむしろ僅かにプラス
- **中立（どの足も方向感なし）のみ1段格下げ** … 唯一ドリフト調整後も有意

### 今後この種の検証をするときの必須手順
1. **必ず基準線（何もしない場合のリターン）を引く。** 上昇相場では
   買いシグナルが良く見えるのは当たり前で、差し引かないと実力が測れない
2. **基準線は銘柄ごとに引く。** 全体平均だと成長株の分が他に混ざる
3. **n<100の結果は信じない。** 有意性検定を必ず通す
4. **利用者向けの文面に検証由来の数字を書くときは、調整後の値を使う。**
   一度「+1.05%」と書いて配信しかけた（ドリフト込みの誤った数字だった）
