...
## 🔬 マルチタイムフレームの実証（2026-08-18・src/mtf_validation.py）

朝に入れたMTF判定を、同日中に過去データで検証したところ**実装が間違っていた**ため修正した。

**検証方法**: 23銘柄（指数・為替・個別株・商品）×5年、8種のシグナル3,855件。
その日までの終値だけで判定（週足は日足から合成。週足データを直接使うと
週の途中でその週の確定値を見てしまい未来を覗くことになる）。
差が偶然かどうかは並べ替え検定（5,000回）で確認。

| グループ | 5日後 | 10日後 | 20日後 | 判定 |
|---|---|---|---|---|
| 完全一致 (n=2257) | +0.28% ✅ | +0.43% ✅ | +1.05% ✅ | **全期間で有意に良い** |
| 一致 (n=778) | -0.15% | -0.42% | -0.20% | 差なし |
| **中立 (n=265)** | **-0.72% ✅** | **-1.03% ✅** | **-1.92% ✅** | **最も悪い** |
| 逆行 (n=555) | +0.03% | +0.24% | -0.81% ✅ | 短期は差なし |

**判明した実装ミス**: 当初は「逆行なら格下げ・中立は放置」としていたが、
逆行は5日/10日で有意差が無く（p=0.89 / 0.36）、実際に最も悪いのは
**時間軸に方向感がない「中立」**だった。理論で決めた格下げ先を実測に置き換えた。

**⚠️ 小サンプルの罠**: 最初に6銘柄で試したとき「完全一致は逆行より+1.87%良い」
と出たが、23銘柄に増やすと差はほぼ消えた（-0.03%）。n=33の偶然だった。
**数字の大小だけで判断せず、必ず件数と検定を確認すること。**
